...
mannen <- subset(df, df$Sex==0) %>% mutate(logRBC = log(RBC))
...
#model for male
modelinm <- glm(irondef~Age+Hb+HPfreqdonatiem + BMI+ testseizoen + haem + non_haem + HeavyADays + ModADays + DaysW10Min + logRBC + RET + MCV,data = mannen, family = "binomial")
...
mydata <- mannen %>%
  dplyr::select_if(is.numeric) %>% select(Age, Hb, BMI,haem, RBC, non_haem, HeavyADays, ModADays, DaysW10Min, RET, MCV)
...
```{R}
model1man <- glm(irondef~Age+Hb+HPfreqdonatiem + BMI + testseizoen + haem+ non_haem + HeavyADays + ModADays + DaysW10Min+ I(RBC^2) + RBC +RET + MCV,data = trainm, family = "binomial")
model2man <- glm(irondef~Age+Hb+HPfreqdonatiem + BMI + testseizoen + haem+ non_haem + HeavyADays + ModADays + DaysW10Min+ RBC +RET + MCV,data = trainm, family = "binomial")
...
library(broom)
model3man_coef<- as.data.frame(tidy(model1man))

library("writexl")
write_xlsx(model3man_coef, "model3mancoef.xls")

anova1<-anova(model2man, model1man, test = "Chisq")
anova1
```
...
modelinv <- glm(irondef~Age+Hb+HPfreqdonatie + BMI + haem + non_haem + HeavyADays + ModADays + DaysW10Min + (MSK_Score:MenL4M)+RBC + RET + MCV + MenL4M,data = vrouwen, family = "binomial")
...
#check for linearity data
  # Select only numeric predictors
mydata2 <- vrouwen %>%
  dplyr::select_if(is.numeric) %>% select(Age, Hb, HPfreqdonatie, BMI,haem, non_haem, HeavyADays, ModADays, DaysW10Min, MSK_Score, RBC, RET, MCV)
...
model1vrouw <- glm(irondef~Age+Hb+HPfreqdonatie + BMI + testseizoen+ haem+ non_haem + HeavyADays + ModADays + DaysW10Min + (MSK_Score:MenL4M)+ MenL4M +  RBC + RET + MCV ,data = trainv, family = "binomial")
...
library(broom)
model3vrouw_coef<- as.data.frame(tidy(model1vrouw, conf.int = T, exponentiate = T))

library("writexl")
write_xlsx(model3vrouw_coef, "model3vrouwcoef.xls")
...
predictions <- bind_cols(testm, prob) %>% rename(prediction = `...22`)

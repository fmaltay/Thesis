...
#model for male
modelinm <- glm(irondef~Age+Hb+HPfreqdonatiem + BMI+ testseizoen + haem + non_haem + HeavyADays + ModADays + DaysW10Min + RBCm + RET + MCV,data = mannen, family = "binomial")
...
mydata <- mannen %>%
  dplyr::select_if(is.numeric) %>% dplyr::select(Age, Hb, BMI,haem, non_haem, HeavyADays, ModADays, DaysW10Min, RET, MCV)
...
```{R}
model1man <- glm(irondef~Age+Hb+HPfreqdonatiem + BMI + testseizoen + haem+ non_haem + HeavyADays + ModADays + DaysW10Min+ RBCm + RET + MCV,data = trainm, family = "binomial")



step.model <- stepAIC(model1man, direction = "backward", 
                      trace = FALSE)


prob <- predict(step.model,testm, type = "response")
...
library(broom)
model4man_coef <- as.data.frame(tidy(step.model))

library("writexl")
write_xlsx(model4man_coef, "model4mancoef.xls")
...
#check for linearity data
  # Select only numeric predictors
mydata2 <- vrouwen %>%
  dplyr::select_if(is.numeric) %>% dplyr::select(Age, Hb, HPfreqdonatie, BMI,haem, non_haem, HeavyADays, ModADays, DaysW10Min, MSK_Score, RBC, RET, MCV)
...
model1vrouw <- glm(irondef~Age+Hb+HPfreqdonatie + BMI + testseizoen+ haem+ non_haem + HeavyADays + ModADays + DaysW10Min + (MSK_Score:MenL4M)+ RBC + RET + MCV + MenL4M ,data = trainv, family = "binomial")




step.modelv <- stepAIC(model1vrouw, direction = "backward", 
                      trace = FALSE)


probb <- predict(step.modelv,testv, type = "response")
...
library(broom)
model4vrouw_coef <- as.data.frame(tidy(step.modelv))

library("writexl")
write_xlsx(model4vrouw_coef, "model4vrouwcoef.xls")
